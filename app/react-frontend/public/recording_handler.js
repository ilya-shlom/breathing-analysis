...
async function sendData () {
  ...
  try {
    const r = await fetch(`${BASE_API}/cut`, { method: 'POST', body: formData });
...
async function startRecording () {
  ...
  formData.append('sid', sid);
  try {
    const r = await fetch(`${BASE_API}/start`, { method: 'POST', body: formData });
...
async function stopRecording () {
  ...
  const formData = new FormData(document.getElementById('audio-sender'));
    formData.append("sid", sid);
    try {
        const response = await fetch(`${BASE_API}/stop`, {
            method: 'POST',
            body: formData
        });
